...
public class EthernetAnalyzer implements CustomAnalyzer {
    ...
    public String getDst_addr(byte[] ethernetHeader) {
        byte[] dst_addr = BitOperator.parse(ethernetHeader,
                EthernetHeader.DST_ADDR_START_BIT,
                EthernetHeader.DST_ADDR_END_BIT);
        return Beautify.beautify(dst_addr, "hex2");
    }

    public String getSrc_addr(byte[] ethernetHeader) {
        byte[] src_addr = BitOperator.parse(ethernetHeader,
                EthernetHeader.SRC_ADDR_START_BIT,
                EthernetHeader.SRC_ADDR_END_BIT);
        return Beautify.beautify(src_addr, "hex2");
    }

    public String getEthertype(byte[] ethernetHeader) {
        byte[] ethertype = BitOperator.parse(ethernetHeader,
                EthernetHeader.ETHERTYPE_START_BIT,
                EthernetHeader.ETHERTYPE_END_BIT);
        return Hex.encodeHexString(ethertype);
    }

    @Subscribe
    public void analyze(PacketWrapper packetWrapper) {
        if (Protocol.get("ETHERNET")
