...
public class IPv4Analyzer implements CustomAnalyzer {
    ...
    public byte getVersion(byte[] ipv4Header) {
        byte[] version = BitOperator.parse(ipv4Header,
                IPv4Header.VERSION_START_BIT, IPv4Header.VERSION_END_BIT);
        byte returnVar = ByteOperator.parseBytesbyte(version);
        return returnVar;
    }

    public byte getIhl(byte[] ipv4Header) {
        byte[] ihl = BitOperator.parse(ipv4Header, IPv4Header.IHL_START_BIT,
                IPv4Header.IHL_END_BIT);
        byte returnVar = ByteOperator.parseBytesbyte(ihl);
        return returnVar;
    }

    public short getDiffserv(byte[] ipv4Header) {
        byte[] diffserv = BitOperator.parse(ipv4Header,
                IPv4Header.DIFFSERV_START_BIT, IPv4Header.DIFFSERV_END_BIT);
        short returnVar = ByteOperator.parseBytesshort(diffserv);
        return returnVar;
    }

    public int getTotalLen(byte[] ipv4Header) {
        byte[] totallen = BitOperator.parse(ipv4Header,
                IPv4Header.TOTALLEN_START_BIT, IPv4Header.TOTALLEN_END_BIT);
        int returnVar = ByteOperator.parseBytesint(totallen);
        return returnVar;
    }

    public int getIdentification(byte[] ipv4Header) {
        byte[] identification = BitOperator.parse(ipv4Header,
                IPv4Header.IDENTIFICATION_START_BIT,
                IPv4Header.IDENTIFICATION_END_BIT);
        int returnVar = ByteOperator.parseBytesint(identification);
        return returnVar;
    }

    public byte getFlags(byte[] ipv4Header) {
        byte[] flags = BitOperator.parse(ipv4Header, IPv4Header.FLAGS_START_BIT,
                IPv4Header.FLAGS_END_BIT);
        byte returnVar = ByteOperator.parseBytesbyte(flags);
        return returnVar;
    }

    public short getFragOffset(byte[] ipv4Header) {
        byte[] fragoffset = BitOperator.parse(ipv4Header,
                IPv4Header.FRAGOFFSET_START_BIT, IPv4Header.FRAGOFFSET_END_BIT);
        short returnVar = ByteOperator.parseBytesshort(fragoffset);
        return returnVar;
    }

    public short getTtl(byte[] ipv4Header) {
        byte[] ttl = BitOperator.parse(ipv4Header, IPv4Header.TTL_START_BIT,
                IPv4Header.TTL_END_BIT);
        short returnVar = ByteOperator.parseBytesshort(ttl);
        return returnVar;
    }

    public String getProtocol(byte[] ipv4Header) {
        byte[] protocol = BitOperator.parse(ipv4Header,
                IPv4Header.PROTOCOL_START_BIT, IPv4Header.PROTOCOL_END_BIT);
        return Hex.encodeHexString(protocol);
    }

    public String getHdrChecksum(byte[] ipv4Header) {
        byte[] hdrchecksum = BitOperator.parse(ipv4Header,
                IPv4Header.HDRCHECKSUM_START_BIT,
                IPv4Header.HDRCHECKSUM_END_BIT);
        return Beautify.beautify(hdrchecksum, "hex");
    }

    public String getSrcAddr(byte[] ipv4Header) {
        byte[] srcaddr = BitOperator.parse(ipv4Header,
                IPv4Header.SRCADDR_START_BIT, IPv4Header.SRCADDR_END_BIT);
        return Beautify.beautify(srcaddr, "ip4");
    }

    public String getDstAddr(byte[] ipv4Header) {
        byte[] dstaddr = BitOperator.parse(ipv4Header,
                IPv4Header.DSTADDR_START_BIT, IPv4Header.DSTADDR_END_BIT);
        return Beautify.beautify(dstaddr, "ip4");
    }

    @Subscribe
    public void analyze(PacketWrapper packetWrapper) {
        if (Protocol.get("IPV4")
