...
public class TcpAnalyzer implements CustomAnalyzer {
    ...
    public int getSrcPort(byte[] tcpHeader) {
        byte[] srcport = BitOperator.parse(tcpHeader,
                TcpHeader.SRCPORT_START_BIT, TcpHeader.SRCPORT_END_BIT);
        int returnVar = ByteOperator.parseBytesint(srcport);
        return returnVar;
    }

    public int getDstPort(byte[] tcpHeader) {
        byte[] dstport = BitOperator.parse(tcpHeader,
                TcpHeader.DSTPORT_START_BIT, TcpHeader.DSTPORT_END_BIT);
        int returnVar = ByteOperator.parseBytesint(dstport);
        return returnVar;
    }

    public long getSeqNo(byte[] tcpHeader) {
        byte[] seqno = BitOperator.parse(tcpHeader, TcpHeader.SEQNO_START_BIT,
                TcpHeader.SEQNO_END_BIT);
        long returnVar = ByteOperator.parseByteslong(seqno);
        return returnVar;
    }

    public long getAckNo(byte[] tcpHeader) {
        byte[] ackno = BitOperator.parse(tcpHeader, TcpHeader.ACKNO_START_BIT,
                TcpHeader.ACKNO_END_BIT);
        long returnVar = ByteOperator.parseByteslong(ackno);
        return returnVar;
    }

    public byte getDataOffset(byte[] tcpHeader) {
        byte[] dataoffset = BitOperator.parse(tcpHeader,
                TcpHeader.DATAOFFSET_START_BIT, TcpHeader.DATAOFFSET_END_BIT);
        byte returnVar = ByteOperator.parseBytesbyte(dataoffset);
        return returnVar;
    }

    public byte getRes(byte[] tcpHeader) {
        byte[] res = BitOperator.parse(tcpHeader, TcpHeader.RES_START_BIT,
                TcpHeader.RES_END_BIT);
        byte returnVar = ByteOperator.parseBytesbyte(res);
        return returnVar;
    }

    public String getFlags(byte[] tcpHeader) {
        byte[] flags = BitOperator.parse(tcpHeader, TcpHeader.FLAGS_START_BIT,
                TcpHeader.FLAGS_END_BIT);
        return Beautify.beautify(flags, "hex");
    }

    public int getWindow(byte[] tcpHeader) {
        byte[] window = BitOperator.parse(tcpHeader, TcpHeader.WINDOW_START_BIT,
                TcpHeader.WINDOW_END_BIT);
        int returnVar = ByteOperator.parseBytesint(window);
        return returnVar;
    }

    public String getChecksum(byte[] tcpHeader) {
        byte[] checksum = BitOperator.parse(tcpHeader,
                TcpHeader.CHECKSUM_START_BIT, TcpHeader.CHECKSUM_END_BIT);
        return Beautify.beautify(checksum, "hex");
    }

    public int getUrgentPtr(byte[] tcpHeader) {
        byte[] urgentptr = BitOperator.parse(tcpHeader,
                TcpHeader.URGENTPTR_START_BIT, TcpHeader.URGENTPTR_END_BIT);
